module governance::voting {
    ...
    const DESCRIPTION_PROPOSAL_KEY: vector<u8> = b"DESCRIPTION_PROPOSAL_KEY";
    ...
    const DESCRIPTION_KEY: vector<u8> = b"DESCRIPTION_KEY";
    ...
    public fun vote<ProposalType: store + copy>(
        _proof: &ProposalType,
        voting_forum_address: address,
        proposal_id: u64,
        num_votes: u64,
    ...
    #[view]
    public fun get_description<ProposalType: store>(
        voting_forum_address: address,
        proposal_id: u64,
    ): vector<u8> acquires VotingForum {
        let voting_forum = borrow_global<VotingForum<ProposalType>>(voting_forum_address);
        let proposal = table::borrow(&voting_forum.proposals, proposal_id);
        let desc = simple_map::borrow(&proposal.metadata, &utf8(DESCRIPTION_PROPOSAL_KEY));
        *desc
    }

    #[view]
    public fun get_description_key(): vector<u8> {
        DESCRIPTION_PROPOSAL_KEY
    }

module governance::governance {
    ...
    struct SimpleTextProposal has store, drop, copy {
        text: String
    }
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@governance)) {
            move_to(
                aptos_framework,
        ...
        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@governance).signer_caps;
    ...
    public entry fun initialize(
        ...
        voting::register<SimpleTextProposal>(governance_admin);
        move_to(governance_admin, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(governance_admin, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(governance_admin),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(governance_admin),
            vote_events: account::new_event_handle<VoteEvent>(governance_admin),
        });
        move_to(governance_admin, VotingRecords {
            votes: table::new(),
        });
        move_to(governance_admin, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        // todo assert admin
        let governance_config = borrow_global_mut<GovernanceConfig>(@governance);
        ...
        let events = borrow_global_mut<GovernanceEvents>(@governance);
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@governance).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@governance).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@governance).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@governance);
    ...
    public fun get_remaining_voting_power(
        ...
        let proposal_expiration = voting::get_proposal_expiration_secs<SimpleTextProposal>(@governance, proposal_id);
        // let lockup_until = stake::get_lockup_secs(stake_pool); todo belkin
        let lockup_until = timestamp::now_seconds() * 2;
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global<VotingRecordsV2>(@governance);
    ...
    public entry fun create_text_proposal(
        ...
        create_proposal_v2_impl(proposer,
            text,
            @governance,
            b"dummy_hash",
            b"metadata_location",
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@governance);
        let _stake_balance = get_voting_power(stake_pool);
        // assert!(
        ...
        simple_map::add(&mut proposal_metadata, utf8(voting::get_description_key()), *string::bytes(&text));
        ...
        // }; todo belkin early resolution

        let proposal_id = voting::create_proposal_v2(
            proposer_address,
            @governance,
            SimpleTextProposal { text },
            execution_hash,
            governance_config.min_voting_threshold,
            proposal_expiration,
            option::none(), // early_resolution_vote_threshold, todo belkin
            proposal_metadata,
    ...
    fun vote_internal(
        ...
        // assert!(stake::get_delegated_voter(stake_pool) == voter_address, error::invalid_argument(ENOT_DELEGATED_VOTER)); todo belkin

        // The voter's stake needs to be locked up at least as long as the proposal's expiration.
        // let proposal_expiration = voting::get_proposal_expiration_secs<GovernanceProposal>(@governance, proposal_id);
        // assert!( // todo belkin
        //     stake::get_lockup_secs(stake_pool) >= proposal_expiration,
        //     error::invalid_argument(EINSUFFICIENT_STAKE_LOCKUP),
        // );
        ...
        voting::vote<SimpleTextProposal>(
            &SimpleTextProposal {
                text: string::utf8(vector::empty<u8>()) // empty - only for type check
            },
            @governance,
            proposal_id,
            voting_power,
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global_mut<VotingRecordsV2>(@governance);
            ...
            *used_voting_power = *used_voting_power + voting_power;
            ...
            let voting_records = borrow_global_mut<VotingRecords>(@governance);
            assert!(
                !table::contains(&voting_records.votes, record_key),
        ...
        let proposal_state = voting::get_proposal_state<SimpleTextProposal>(@governance, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@governance);
        ...
        let execution_hash = voting::get_execution_hash<SimpleTextProposal>(@governance, proposal_id);
    ...
    public fun resolve(
        ...
        voting::resolve<SimpleTextProposal>(@governance, proposal_id);
    ...
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<SimpleTextProposal>(@governance, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<SimpleTextProposal>(@governance, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@governance).hashes;
    ...
    public fun get_voting_power(_pool_address: address): u64 {
        // todo belkin
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@governance);
    ...
    fun assert_voting_initialization() {
        if (features::partial_governance_voting_enabled()) {
            assert!(exists<VotingRecordsV2>(@governance), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
        };
    }
    ...
    public entry fun create_proposal_for_test(
        ...
        create_text_proposal(
            proposer,
            string::utf8(b"text text")
        );
    }

    #[test_only]
    public entry fun resolve_proposal_for_test(
        proposal_id: u64,
        signer_address: address,
        multi_step: bool,
    ...
    public entry fun test_voting_generic(
        ...
        create_proposal_for_test(&proposer,);
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<SimpleTextProposal>(
        ...
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@governance).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @governance, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @governance, 1);
        assert!(voting::is_resolved<SimpleTextProposal>(@governance, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@governance).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 3);
    }

    #[test(aptos_framework = @governance, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(aptos_framework, proposer, yes_voter, no_voter,  false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(aptos_framework, proposer, yes_voter, no_voter,  true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            voting::resolve_proposal_v2<SimpleTextProposal>(@governance, 0, next_execution_hash);
            assert!(voting::is_resolved<SimpleTextProposal>(@governance, 0), 0);
            if (vector::length(&next_execution_hash) == 0) {
            ...
            voting::resolve<SimpleTextProposal>(@governance, 0);
            assert!(voting::is_resolved<SimpleTextProposal>(@governance, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@governance).hashes;
    ...
    #[test(aptos_framework = @governance, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    ...
    public fun setup_voting(
        ...
        store_signer_cap(
            aptos_framework,
            @governance,
            account::create_test_signer_cap(@governance),
        );
    ...
    #[test(aptos_framework = @governance)]
    public entry fun test_update_governance_config(
        ...
        let config = borrow_global<GovernanceConfig>(@governance);
